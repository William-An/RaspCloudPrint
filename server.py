...
class Upload:
    ...
    def POST(self):
        ...
        if 'printjob' in x:                                   # to check if the file-object is created
            filepath=x.printjob.filename.replace('\\','/')    # replaces the windows-style slashes with linux ones.
...
class Log:
	def GET(self):
		with open("static/log") as logfile:
			return "<a href=\"static/log\">Download Log</a><br>"+"<br>".join(logfile.readlines());
def notfound():
	raise web.seeother('/error')
...
def printer(filename):
	with open('static/log','a+') as logfile:
		...
		try:
			subprocess.run(['lp',filename],timeout=30)
